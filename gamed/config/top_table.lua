...
rank_list[26] = {id =26, type = RANK_TYPE_PERSON}   	--个人太昊排行 
rank_list[29] = {id =29, type = RANK_TYPE_PERSON}   	--个人焚香谷排行
rank_list[25] = {id =25, type = RANK_TYPE_PERSON}   	--个人辰皇排行
rank_list[208] = {id =208, type = RANK_TYPE_PERSON}   --个人青罗排行
rank_list[210] = {id =210, type = RANK_TYPE_PERSON}   --个人牵机排行（个人声望排行从209往后排）
rank_list[211] = {id =211, type = RANK_TYPE_PERSON}   --个人英招排行 
rank_list[212] = {id =212, type = RANK_TYPE_PERSON}   --个人破军排行
rank_list[214] = {id =214, type = RANK_TYPE_PERSON}   --个人青罗排行
rank_list[215] = {id =215, type = RANK_TYPE_PERSON}   --个人青罗排行
rank_list[217] = {id =217, type = RANK_TYPE_PERSON}   --个人青罗排行
...
rank_list[56] = {id =56, type = RANK_TYPE_FACTION}   --帮派太昊排行 
rank_list[59] = {id =59, type = RANK_TYPE_FACTION}   --帮派焚香谷排行
rank_list[55] = {id =55, type = RANK_TYPE_FACTION}   --帮派辰皇排行
rank_list[310] = {id =310, type = RANK_TYPE_FACTION} --帮派牵机排行
rank_list[311] = {id =311, type = RANK_TYPE_FACTION} --帮派英招排行
rank_list[312] = {id =312, type = RANK_TYPE_FACTION} --帮派破军排行
rank_list[314] = {id =314, type = RANK_TYPE_FACTION} --帮派青罗排行
rank_list[315] = {id =315, type = RANK_TYPE_FACTION} --帮派青罗排行
rank_list[317] = {id =317, type = RANK_TYPE_FACTION} --帮派青罗排行
...
local function grant_title()
	...
	local shidezhi_rank = 1                 --开启师德值排行称谓的rank
	local shidezhi_high = 10000             --开启师德值排名称谓的师德值
	local shidezhi_low = 500                --获得师德值排名称谓的限制
	local daoxin_rank = 50                  --开启道心值排行称谓的rank
	local daoxin_high = 10000               --开启道心值排行称谓的道心值
  ...
  local qingyuan_rank = 50								--开启个人情缘排名称谓的rank
	...
	local taihaoshengwang = get_rank_table(26)     			--个人太昊声望排行 
	local fenxiangshengwang = get_rank_table(29)     		--个人焚香谷声望排行
	local chenhuangshengwang = get_rank_table(25)    		--个人辰皇声望排行
	local qianjishengwang = get_rank_table(210)    		  --个人牵机声望排行
	local yingzhaoshengwang = get_rank_table(211)    		--个人英招声望排行
	local pojunshengwang = get_rank_table(212)    			--个人破军声望排行
	local qingluoshengwang = get_rank_table(214)    		--个人青罗声望排行
	local gevrinhonor = get_rank_table(215)    		--comper
	local syliahonor = get_rank_table(217)    		--comper
	
	local shidezhi = get_rank_table(18)           			--师德值排行
	local daoxin = get_rank_table(19)                   --道心值排行
	local moxing = get_rank_table(20)                   --佛缘值排行
	local foyuan = get_rank_table(21)	                  --魔性值排行
	...
	local huaiguangbangpai = get_rank_table(57)			--帮派怀光声望排行 
	local tianhuabangpai = get_rank_table(58)				--帮派天华声望排行
	local taihaobangpai = get_rank_table(56)				--帮派太昊声望排行 
	local fenxiangbangpai = get_rank_table(59)			--帮派焚香谷声望排行
	local chenhuangbangpai = get_rank_table(55)			--帮派辰皇声望排行
	local qianjibangpai = get_rank_table(310)				--帮派牵机声望排行
	local yingzhaobangpai = get_rank_table(311)			--帮派英招声望排行	
	local pojunbangpai = get_rank_table(312)				--帮派破军声望排行	
	local qingluobangpai = get_rank_table(314)			--帮派青罗声望排行
	local gevrinallliancerank = get_rank_table(315)			--comper
	local syliaallliancerank = get_rank_table(317)			--comper
	...
	if taihaoshengwang and taihaoshengwang[m] then
			if taihaoshengwang[m].rank == shengwang_rank then
				val[20] = taihaoshengwang[m].val
			end
		end 
		
	if fenxiangshengwang and fenxiangshengwang[m] then
			if fenxiangshengwang[m].rank == shengwang_rank then
				val[21] = fenxiangshengwang[m].val
			end
		end

	if chenhuangshengwang and chenhuangshengwang[m] then
			if chenhuangshengwang[m].rank == shengwang_rank then
				val[22] = chenhuangshengwang[m].val
			end
		end
		
	if qianjishengwang and qianjishengwang[m] then
			if qianjishengwang[m].rank == shengwang_rank then
				val[23] = qianjishengwang[m].val
			end
		end
		
	if yingzhaoshengwang and yingzhaoshengwang[m] then
			if yingzhaoshengwang[m].rank == shengwang_rank then
				val[24] = yingzhaoshengwang[m].val
			end
		end
		
	if pojunshengwang and pojunshengwang[m] then
			if pojunshengwang[m].rank == shengwang_rank then
				val[25] = pojunshengwang[m].val
			end
		end
	if qingluoshengwang and qingluoshengwang[m] then
			if qingluoshengwang[m].rank == shengwang_rank then
				val[26] = qingluoshengwang[m].val
			end
		end
	if gevrinhonor and gevrinhonor[m] then
			if gevrinhonor[m].rank == shengwang_rank then
				val[27] = gevrinhonor[m].val
			end
		end
	if syliahonor and syliahonor[m] then
			if syliahonor[m].rank == shengwang_rank then
				val[28] = syliahonor[m].val
			end
		end
	...
	for k, v in pairs(person_title) do					--发放个人称谓及条件限制
		...
		if taihaoshengwang and taihaoshengwang[k] then
			if val[20] ~= nil and val[20] >= shengwang_high then
				if taihaoshengwang[k].rank <= 1 then
					add_title(v, title_definition['太昊声望排行1'].id)
				elseif taihaoshengwang[k].rank <= 5 and taihaoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['太昊声望排行2-5'].id)
				elseif taihaoshengwang[k].rank <= 10 and taihaoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['太昊声望排行6-10'].id)
				elseif taihaoshengwang[k].rank <= 100 and taihaoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['太昊声望排行11-50'].id)
				elseif taihaoshengwang[k].rank <= 500 and taihaoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['太昊声望排行51-400'].id)
				end
			end
		end 
		
		if fenxiangshengwang and fenxiangshengwang[k] then
			if val[21] ~= nil and val[21] >= shengwang_high then
				if fenxiangshengwang[k].rank <= 1 then
					add_title(v, title_definition['焚香谷声望排行1'].id)
				elseif fenxiangshengwang[k].rank <= 5 and fenxiangshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['焚香谷声望排行2-5'].id)
				elseif fenxiangshengwang[k].rank <= 10 and fenxiangshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['焚香谷声望排行6-10'].id)
				elseif fenxiangshengwang[k].rank <= 100 and fenxiangshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['焚香谷声望排行11-50'].id)
				elseif fenxiangshengwang[k].rank <= 500 and fenxiangshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['焚香谷声望排行51-400'].id)
				end
			end
		end

		if chenhuangshengwang and chenhuangshengwang[k] then
			if val[22] ~= nil and val[22] >= shengwang_high then
				if chenhuangshengwang[k].rank <= 1 then
					add_title(v, title_definition['辰皇声望排行1'].id)
				elseif chenhuangshengwang[k].rank <= 5 and chenhuangshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['辰皇声望排行2-5'].id)
				elseif chenhuangshengwang[k].rank <= 10 and chenhuangshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['辰皇声望排行6-10'].id)
				elseif chenhuangshengwang[k].rank <= 100 and chenhuangshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['辰皇声望排行11-50'].id)
				elseif chenhuangshengwang[k].rank <= 500 and chenhuangshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['辰皇声望排行51-400'].id)
				end
			end
		end
		
		if qianjishengwang and qianjishengwang[k] then
			if val[23] ~= nil and val[23] >= shengwang_high then
				if qianjishengwang[k].rank <= 1 then
					add_title(v, title_definition['牵机族声望排行1'].id)
				elseif qianjishengwang[k].rank <= 5 and qianjishengwang[k].val >= shengwang_low then
					add_title(v, title_definition['牵机族声望排行2-5'].id)
				elseif qianjishengwang[k].rank <= 10 and qianjishengwang[k].val >= shengwang_low then
					add_title(v, title_definition['牵机族声望排行6-10'].id)
				elseif qianjishengwang[k].rank <= 100 and qianjishengwang[k].val >= shengwang_low then
					add_title(v, title_definition['牵机族声望排行11-50'].id)
				elseif qianjishengwang[k].rank <= 500 and qianjishengwang[k].val >= shengwang_low then
					add_title(v, title_definition['牵机族声望排行51-400'].id)
				end
			end
		end 
		
		if yingzhaoshengwang and yingzhaoshengwang[k] then
			if val[24] ~= nil and val[24] >= shengwang_high then
				if yingzhaoshengwang[k].rank <= 1 then
					add_title(v, title_definition['英招族声望排行1'].id)
				elseif yingzhaoshengwang[k].rank <= 5 and yingzhaoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['英招族声望排行2-5'].id)
				elseif yingzhaoshengwang[k].rank <= 10 and yingzhaoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['英招族声望排行6-10'].id)
				elseif yingzhaoshengwang[k].rank <= 100 and yingzhaoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['英招族声望排行11-50'].id)
				elseif yingzhaoshengwang[k].rank <= 500 and yingzhaoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['英招族声望排行51-400'].id)
				end
			end
		end 
		
		if pojunshengwang and pojunshengwang[k] then
			if val[25] ~= nil and val[25] >= shengwang_high then
				if pojunshengwang[k].rank <= 1 then
					add_title(v, title_definition['破军族声望排行1'].id)
				elseif pojunshengwang[k].rank <= 5 and pojunshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['破军族声望排行2-5'].id)
				elseif pojunshengwang[k].rank <= 10 and pojunshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['破军族声望排行6-10'].id)
				elseif pojunshengwang[k].rank <= 100 and pojunshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['破军族声望排行11-50'].id)
				elseif pojunshengwang[k].rank <= 500 and pojunshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['破军族声望排行51-400'].id)
				end
			end
		end 
		
		if qingluoshengwang and qingluoshengwang[k] then
			if val[26] ~= nil and val[26] >= shengwang_high then
				if qingluoshengwang[k].rank <= 1 then
					add_title(v, title_definition['青罗族声望排行1'].id)
				elseif qingluoshengwang[k].rank <= 5 and qingluoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['青罗族声望排行2-5'].id)
				elseif qingluoshengwang[k].rank <= 10 and qingluoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['青罗族声望排行6-10'].id)
				elseif qingluoshengwang[k].rank <= 100 and qingluoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['青罗族声望排行11-50'].id)
				elseif qingluoshengwang[k].rank <= 500 and qingluoshengwang[k].val >= shengwang_low then
					add_title(v, title_definition['青罗族声望排行51-400'].id)
				end
			end
		end
		
		if gevrinhonor and gevrinhonor[k] then
			if val[27] ~= nil and val[27] >= shengwang_high then
				if gevrinhonor[k].rank <= 1 then
					add_title(v, title_definition['归云山声望排行1'].id)
				elseif gevrinhonor[k].rank <= 5 and gevrinhonor[k].val >= shengwang_low then
					add_title(v, title_definition['归云山声望排行2-5'].id)
				elseif gevrinhonor[k].rank <= 10 and gevrinhonor[k].val >= shengwang_low then
					add_title(v, title_definition['归云山声望排行6-10'].id)
				elseif gevrinhonor[k].rank <= 100 and gevrinhonor[k].val >= shengwang_low then
					add_title(v, title_definition['归云山声望排行11-50'].id)
				elseif gevrinhonor[k].rank <= 500 and gevrinhonor[k].val >= shengwang_low then
					add_title(v, title_definition['归云山声望排行51-400'].id)
				end
			end
		end
		
		if syliahonor and syliahonor[k] then
			if val[28] ~= nil and val[28] >= shengwang_high then
				if syliahonor[k].rank <= 1 then
					add_title(v, title_definition['画影殿声望排行1'].id)
				elseif syliahonor[k].rank <= 5 and syliahonor[k].val >= shengwang_low then
					add_title(v, title_definition['画影殿声望排行2-5'].id)
				elseif syliahonor[k].rank <= 10 and syliahonor[k].val >= shengwang_low then
					add_title(v, title_definition['画影殿声望排行6-10'].id)
				elseif syliahonor[k].rank <= 100 and syliahonor[k].val >= shengwang_low then
					add_title(v, title_definition['画影殿声望排行11-50'].id)
				elseif syliahonor[k].rank <= 500 and syliahonor[k].val >= shengwang_low then
					add_title(v, title_definition['画影殿声望排行51-400'].id)
				end
			end
		end
		...
		if tianhuabangpai and tianhuabangpai[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if tianhuabangpai[k].rank <= bangpai_rank and tianhuabangpai[k].val >= bangpai_shengwang then
					person_title[tianhuabangpai[k].prof_master] = add_title(person_title[tianhuabangpai[k].prof_master],title_definition['天华氏第一帮'].id)
				end
			end
		end 
		...
		if huaiguangbangpai and huaiguangbangpai[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if huaiguangbangpai[k].rank <= bangpai_rank and huaiguangbangpai[k].val >= bangpai_shengwang then
					person_title[huaiguangbangpai[k].prof_master] = add_title(person_title[huaiguangbangpai[k].prof_master],title_definition['怀光氏第一帮'].id)
				end
			end
		end
		
		if taihaobangpai and taihaobangpai[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if taihaobangpai[k].rank <= bangpai_rank and taihaobangpai[k].val >= bangpai_shengwang then
					person_title[taihaobangpai[k].prof_master] = add_title(person_title[taihaobangpai[k].prof_master],title_definition['太昊氏第一帮'].id)
				end
			end
		end 
		
		if fenxiangbangpai and fenxiangbangpai[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if fenxiangbangpai[k].rank <= bangpai_rank and fenxiangbangpai[k].val >= bangpai_shengwang then
					person_title[fenxiangbangpai[k].prof_master] = add_title(person_title[fenxiangbangpai[k].prof_master],title_definition['焚香谷第一帮'].id)
				end
			end
		end

		if chenhuangbangpai and chenhuangbangpai[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if chenhuangbangpai[k].rank <= bangpai_rank and chenhuangbangpai[k].val >= bangpai_shengwang then
					person_title[chenhuangbangpai[k].prof_master] = add_title(person_title[chenhuangbangpai[k].prof_master],title_definition['辰皇氏第一帮'].id)
				end
			end
		end

		if qianjibangpai and qianjibangpai[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if qianjibangpai[k].rank <= bangpai_rank and qianjibangpai[k].val >= bangpai_shengwang then
					person_title[qianjibangpai[k].prof_master] = add_title(person_title[qianjibangpai[k].prof_master],title_definition['牵机族第一帮'].id)
				end
			end
		end
		
		if yingzhaobangpai and yingzhaobangpai[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if yingzhaobangpai[k].rank <= bangpai_rank and yingzhaobangpai[k].val >= bangpai_shengwang then
					person_title[yingzhaobangpai[k].prof_master] = add_title(person_title[yingzhaobangpai[k].prof_master],title_definition['英招族第一帮'].id)
				end
			end
		end		

		if pojunbangpai and pojunbangpai[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if pojunbangpai[k].rank <= bangpai_rank and pojunbangpai[k].val >= bangpai_shengwang then
					person_title[pojunbangpai[k].prof_master] = add_title(person_title[pojunbangpai[k].prof_master],title_definition['破军族第一帮'].id)
				end
			end
		end	

		if qingluobangpai and qingluobangpai[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if qingluobangpai[k].rank <= bangpai_rank and qingluobangpai[k].val >= bangpai_shengwang then
					person_title[qingluobangpai[k].prof_master] = add_title(person_title[qingluobangpai[k].prof_master],title_definition['青罗宫第一帮'].id)
				end
			end
		end
		
		if gevrinallliancerank and gevrinallliancerank[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if gevrinallliancerank[k].rank <= bangpai_rank and gevrinallliancerank[k].val >= bangpai_shengwang then
					person_title[gevrinallliancerank[k].prof_master] = add_title(person_title[gevrinallliancerank[k].prof_master],title_definition['归云山第一帮'].id)
				end
			end
		end
		
		if syliaallliancerank and syliaallliancerank[k] then
			if b_val[1] ~= nil and b_val[1] == bangpai_sum then
				if syliaallliancerank[k].rank <= bangpai_rank and syliaallliancerank[k].val >= bangpai_shengwang then
					person_title[syliaallliancerank[k].prof_master] = add_title(person_title[syliaallliancerank[k].prof_master],title_definition['画影殿第一帮'].id)
				end
			end
		end
