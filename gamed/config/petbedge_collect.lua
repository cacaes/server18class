...
function ZLuaPetCollectTypes()
	local CollectTypes={}
	CollectTypes["zhongzhi"]		=1
	CollectTypes["famu"]				=2
	CollectTypes["shoulie"]			=3
	CollectTypes["diaoyu"]			=4
	CollectTypes["caikuang"]		=5
	CollectTypes["kaogu"]				=6
	return CollectTypes
end 
...
function PetBedgeCollect(pet_tid,pet,collecttype,mode)
  ...
  ItemList[3][6] = {16830,16873,16916,15959,16999,17049}
	...
	local Acuity=pet:QueryData(DataIndex["acuity"]) or 0
	local Stamina=pet:QueryData(DataIndex["stamina"]) or 0
	local Dowry=pet:QueryData(DataIndex["dowry"]) or 0
	local Wisdom=pet:QueryData(DataIndex["wisdom"]) or 0
	local CNR=pet:QueryData(DataIndex["cnr"]) or 0
	local Sawy=pet:QueryData(DataIndex["sawy"]) or 0
