...
function ZLuaPetBedgeElementInfo(pet_tid,origin)
  if ZLuaPetBedgeExistInTmplIDs(pet_tid) == 0 then 
	...
	elseif pet_tid==AvailablePets["meixindiejing"].pet_tid then 
		...
		elementchance={100,100,100,100,100,100}
	elseif pet_tid==AvailablePets["songcaijinshu"].pet_tid or pet_tid==AvailablePets["H_shouhushiling"].pet_tid then 
		elementlist={0,8,10,13,15,17}
		elementchance={100,100,100,100,100,100}
	elseif pet_tid==AvailablePets["shouhushiling"].pet_tid or pet_tid==AvailablePets["Q_shouhushiling"].pet_tid or pet_tid==AvailablePets["N_shouhushiling"].pet_tid or pet_tid==AvailablePets["H_songcaijinshu"].pet_tid or pet_tid==AvailablePets["guangminghuo"].pet_tid then 
...
function PetBedgeTalk(pet_tid,pet)
  ...
  if ZLuaPetBedgeExistInTmplIDs(pet_tid) == 0 then 
	...
	elseif pet_tid==AvailablePets["shouhushiling"].pet_tid or pet_tid==AvailablePets["Q_shouhushiling"].pet_tid or pet_tid==AvailablePets["N_shouhushiling"].pet_tid or pet_tid==AvailablePets["H_songcaijinshu"].pet_tid then 
	...
	elseif pet_tid==AvailablePets["S_biyao"].pet_tid then ---//待修改
		...
		talk[4]={}
		talk[5]={}
	elseif pet_tid==AvailablePets["baihu"].pet_tid or pet_tid==AvailablePets["Z_baihu"].pet_tid or pet_tid == 41784 then 
	...
	if petelement==Element["danqie"] then ---//胆怯
		talk[1]={5101,5102,5103,5104,5105,5106,5107,5108,5109,5110,5286,2628,5297}
	elseif petelement==Element["lumang"] then---//鲁莽
		talk[1]={5111,5112,5113,5114,5115,5116,5117,5118,5119,5120,1110,1131,2401,2904,5303,5409}
	elseif petelement==Element["kuangye"] then---//狂野
		talk[1]={5121,5122,5123,5132,5132,5132,5127,5128,5129,5130,2310,5303,5411}
	elseif petelement==Element["baozao"] then---//暴躁
		talk[1]={5131,5132,5133,5134,5135,5136,5137,5138,5139,5140,2904,5296,5411}
	elseif petelement==Element["gaoao"] then---//高傲	
		talk[1]={5141,5142,5143,5144,5145,5146,5147,5148,5149,5150,5284,1024,2510,5302,5407}
	elseif petelement==Element["youya"] then	---//优雅
		talk[1]={5151,5152,5153,5154,5155,5156,5157,5158,5159,5160,1337,1330,5406,5410}
	elseif petelement==Element["yaomei"] then	---//妖媚
		talk[1]={5161,5162,5163,5164,5165,5166,5167,5168,5169,5170,3204,5296,5406}
	elseif petelement==Element["wenrou"] then		---//温柔
		talk[1]={5171,5172,5173,5174,5175,5176,5177,5178,5179,5180,5286,5291,5413,5414}
	elseif petelement==Element["yinxian"] then	---//阴险
		talk[1]={5181,5182,5183,5184,5185,5186,5187,5188,5189,5190,1611,1612,1616,1617,1630,1806,1828,5299,5301,5412}
	elseif petelement==Element["xiee"] then	---//邪恶
		talk[1]={5191,5192,5193,5194,5195,5196,5197,5198,5199,5200,1626,1806,1828,5295,5301,5412}
	elseif petelement==Element["wanpi"] then---//顽皮
		talk[1]={5201,5202,5203,5204,5205,5206,5207,5208,5209,5210,5288,5289,5292,1801,5299,5300,5407,5414}
	elseif petelement==Element["weiyan"] then---//威严
		talk[1]={5211,5212,5213,5214,5215,5216,5217,5218,5219,5220,1709,5294,5409}
	elseif petelement==Element["shensheng"] then---//神圣
		talk[1]={5221,5222,5223,5232,5232,5232,5227,5228,5229,5230,2438,5408}
	elseif petelement==Element["ruizhi"] then---//睿智
		talk[1]={5231,5232,5233,5234,5235,5236,5237,5238,5239,5320,5282,5285,5287,5290,5293,2642,5405,5413}
	elseif petelement==Element["miantian"] then---//腼腆
		talk[1]={5321,5322,5323,5324,5325,5326,5327,5328,5329,5320,2628,5297,5410}
	elseif petelement==Element["boxue"] then---//博学
		talk[1]={5321,5322,5323,5324,5325,5326,5327,5328,5329,5320,5287,5290,5293,3205,5405}
	elseif petelement==Element["gupi"] then---//孤僻	
		talk[1]={5321,5322,5323,5324,5325,5326,5327,5328,5329,5270,5283,5286,5298,5304,5410}
	elseif petelement==Element["yiyu"] then---//异域
		talk[1]={5271,5272,5273,5274,5275,5276,5277,5278,5279,5280,1917,5300}
	else
	end 
